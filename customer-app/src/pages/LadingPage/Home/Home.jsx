...
// cspell:ignore Belapur Sunaina Shani Dadar Andheri
  ...
  return (
    <div className="bg-(--background) text-(--text)">
      <section className="relative min-h-[74vh] overflow-hidden pb-12">
        ...
        <div className="absolute inset-0 bg-linear-to-b from-white/70 via-white/45 to-white/86" />

        <div className="relative mx-auto flex max-w-6xl flex-col items-center px-6 pb-16 pt-28 text-center sm:pt-32">
          <h1 className="max-w-3xl font-['Playfair_Display'] text-5xl font-medium leading-[0.95] text-(--secondary) sm:text-6xl lg:text-[86px]">
            Beauty, Wellness, Confidence
          </h1>

          <div className="relative z-10 mt-10 w-full max-w-117.5 translate-y-6 rounded-[2.25rem] border border-[#ece7e1] bg-white/95 p-4 shadow-[0_18px_45px_rgba(0,0,0,0.10)] backdrop-blur">
            <Link
              to="/login"
              className="group flex w-full items-center justify-between rounded-full bg-(--primary) px-9 py-5 text-sm font-semibold uppercase tracking-[0.22em] text-white transition hover:brightness-110"
            >
            ...
            <p className="mt-4 text-[10px] uppercase tracking-[0.26em] text-(--gray-700)">
              Instant Confirmation � Curated Luxury Experts
            </p>
          </div>
        </div>

        <div className="pointer-events-none absolute -bottom-32.5 left-1/2 h-72 w-[145%] -translate-x-1/2 rounded-[50%] bg-(--background)" />
      </section>

      <section className="mx-auto max-w-6xl px-6 py-20">
        <h2 className="mb-6 text-3xl font-bold text-(--text)">Browse Experiences</h2>
      ...
      <section className="mx-auto max-w-6xl px-6 mt-10">
        <div className="grid grid-cols-2 gap-4 rounded-3xl bg-(--gray-100)/90 p-6 shadow-xl ring-1 ring-black/5 sm:grid-cols-4">
          <div className="text-center"><h3 className="text-2xl font-bold text-(--text)">10K+</h3><p className="mt-1 text-sm text-(--gray-700)">Happy Customers</p></div>
          <div className="text-center"><h3 className="text-2xl font-bold text-(--text)">98%</h3><p className="mt-1 text-sm text-(--gray-700)">Satisfaction Rate</p></div>
          <div className="text-center"><h3 className="text-2xl font-bold text-(--text)">5M+</h3><p className="mt-1 text-sm text-(--gray-700)">Haircuts Done</p></div>
          <div className="text-center"><h3 className="text-2xl font-bold text-(--text)">50+</h3><p className="mt-1 text-sm text-(--gray-700)">Expert Stylists</p></div>
        </div>
      </section>
